...
# connect ----------------------------------------- 
  ...
  pheno<-data.table(dbGetQuery(con,
...
# Generate data to set up long data format
  ...
  # then copy to felek
  envData<-data.table(dbGetQuery(con,"SELECT * FROM data_environmental;"))
  ...
  if(modelType=="js"){
    save(dMData, evalList, d,
         file = paste0("~/westbrookJS/",fileName))
    save(dMData,evalList,d,
         file= file.path(processedDir,fileName))
  } else {
    if(modelType=="cjs"){
      save(dMData, evalList,
           file = paste0("~/simpleCJS/",fileName))
      save(dMData,evalList,
           file= file.path(processedDir,fileName))
    }
  }
  ...
  if(modelType=='js'){assign('d',d,envir=.GlobalEnv)}
 # }
