...
wb_ed <- ed[tolower(ed[['temperature_source']]) == 'sec 6 dl',]
wb_ed <- wb_ed[tolower(wb_ed[['discharge_source']]) == 'flow extension',]

## Save cleaned daily data.
dbWriteTable(conn=link$conn, name='data_environmental', value=wb_ed,
...
envLoess<-data.table(ed)[,list(
                 typical_temperature=predict(loess(temperature~day_of_year,span=1/5),
                                             data.frame(day_of_year=1:366)),
                 typical_discharge=predict(loess(log10(discharge)~day_of_year,span=1/4),
                                           data.frame(day_of_year=1:366)),
                 day_of_year=1:366),
           by=river]
# temperature_average <- data.frame(
# 	day_of_year=1:366, 
envLoess[['season']] <- sapply(envLoess[['day_of_year']], day_to_season)
setkey(envLoess,river,day_of_year)
...
ed<-data.table(ed,key=c("river","day_of_year"))
ed<-envLoess[ed]
seasonal_means<-ed[,list(seasonal_mean_log10discharge=mean(log10(discharge),na.rm=T),
                         seasonal_mean_temperature=mean(temperature,na.rm=T)),
                   by=list(river,season)]
setkey(seasonal_means,river,season)
average_ed <- envLoess
setkey(average_ed,river,season)
average_ed<-seasonal_means[average_ed]
setkey(average_ed,river,day_of_year)
...
edj<-data.table(ed,key=c("river","day_of_year"))
setkey(envLoess,river,day_of_year)
edj[,zst:=envLoess[edj,scale(temperature-typical_temperature)[,1]]]
edj[,zsd:=envLoess[edj,scale(log10(discharge)-typical_discharge)[,1]]]

edj[,daily_deviation_from_seasonal_mean_temperature:=temperature-mean(temperature,na.rm=T),by=list(river,season)] 
edj[,zsdm:=scale(daily_deviation_from_seasonal_mean_temperature)[,1],by=river]

edj[,daily_deviation_from_seasonal_mean_discharge:=discharge-mean(discharge,na.rm=T),by=list(river,season)] 
edj[,zstm:=scale(daily_deviation_from_seasonal_mean_discharge)[,1],by=river]
